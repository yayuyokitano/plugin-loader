...
function SidebarButton(props:{
	...
	return (
		<button
			class={styles.sidebarButton + (activeSetting().namei18n === item.namei18n ? " " + styles.active : "")}
...
export default function Sidebar(props:{
	...
	return (
		<nav class={styles.sidebar}>
			<For each={items}>{(item) =>
				<SidebarButton item={item} activeSetting={activeSetting} setActiveSetting={setActiveSetting} />
			}</For>
		</nav>
	)
}
