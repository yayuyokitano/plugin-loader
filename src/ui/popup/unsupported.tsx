...
const ActiveConnector = lazy(async () => {
	...
	setActive(await browser.runtime.sendMessage({ type: "getActiveConnector" }));
	return {default: () => (
		<div>
			<button onClick={async () => {
				const a = await browser.runtime.sendMessage({ type: "getActiveConnector" });
				console.log(a);
				setActive(a);}}>refresh</button>
			<p>{active || "None"}</p>
		</div>
	)};
})
